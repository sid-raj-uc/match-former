...
def plot_triplet(img1_idx, img2_idx, query_points):
    ...
    for (pt_name, (ux, uy)) in query_points.items():
        ...
        # Get Predictions
        pred_v = get_closest_match(input_data_v['mkpts0_f'].cpu().numpy(), input_data_v['mkpts1_f'].cpu().numpy(), np.array([ux, uy]))
        pred_c = get_closest_match(input_data_c['mkpts0_f'].cpu().numpy(), input_data_c['mkpts1_f'].cpu().numpy(), np.array([ux, uy]))
        ...
        plt.tight_layout()
        plt.savefig(f'triplet_{pt_name}.png')
        print(f"Saved triplet_{pt_name}.png")

test_points = {
    'Blank_Wall_Textureless': (550, 100),
    'Repetitive_Keyboard_Keys': (300, 360)
}
plot_triplet(0, 10, test_points)
